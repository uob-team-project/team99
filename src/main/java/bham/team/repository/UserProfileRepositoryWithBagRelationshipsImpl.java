...
public class UserProfileRepositoryWithBagRelationshipsImpl implements UserProfileRepositoryWithBagRelationships {
    ...
    public Optional<UserProfile> fetchBagRelationships(Optional<UserProfile> userProfile) {
        return userProfile.map(this::fetchTeams);
    }
    ...
    public List<UserProfile> fetchBagRelationships(List<UserProfile> userProfiles) {
        return Optional.of(userProfiles).map(this::fetchTeams).orElse(Collections.emptyList());
    }

    UserProfile fetchTeams(UserProfile result) {
        return entityManager
            .createQuery(
                "select userProfile from UserProfile userProfile left join fetch userProfile.teams where userProfile.id = :id",
                UserProfile.class
            )
    ...
    List<UserProfile> fetchTeams(List<UserProfile> userProfiles) {
        ...
        List<UserProfile> result = entityManager
            .createQuery(
                "select userProfile from UserProfile userProfile left join fetch userProfile.teams where userProfile in :userProfiles",
                UserProfile.class
            )
